...
public:
    ...
    void Read(int user_id, int page_count) {
        if (users.find(user_id) != users.end()) {
            if (--rating[users[user_id]] == 0) {
                rating.erase(users[user_id]);
            }
        }
    ...
    double Cheer(int user_id) {
        if (users.find(user_id) == users.end()) return 0;
        if (users.size() == 1) return 1;

        auto iter = rating.find(users[user_id]);
        return Dist(rating.begin(), iter) * 1.0 / (users.size() - 1) / 1.0;
    }

private:

    int Dist(std::map<int, int>::iterator begin, std::map<int, int>::iterator end){
        int dist = 0;
        for (auto i = rating.begin(); i != end; ++i) {
            dist += i->second;
        }
        return dist;
    }
