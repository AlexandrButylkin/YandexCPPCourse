...
private:
    ...
    class IndexProxy {
    public:
        explicit IndexProxy(Polynomial& ptr_, size_t degree_) : ptr(ptr_), degree(degree_) {}
        IndexProxy& operator= (T value){
            while(degree > ptr.coeffs_.size() - 1){
                ptr.coeffs_.emplace_back();
            }
            ptr.coeffs_[degree] = value;
            ptr.Shrink();
            return *this;
        }

        friend bool operator==(const IndexProxy& lhs, const T& rhs) {
            return std::as_const(lhs.ptr)[lhs.degree] == rhs;
        }

        friend std::ostream& operator<<(std::ostream& os, const IndexProxy& lhs){
            os << std::as_const(lhs.ptr)[lhs.degree];
            return os;
        }
