...
public:
    ...
    void CombineWith(const BulkLinearUpdater& other) {
        ...
        add_.delta.incomes = add_.delta.incomes * other.tax_.FACTOR + other.add_.delta.incomes;
        add_.delta.spends += other.add_.delta.spends;
    }

    Money Collapse(Money origin, IndexSegment segment) const {
        return {.incomes = origin.incomes * tax_.FACTOR + add_.delta.incomes * segment.length(),
                .spends = origin.spends + add_.delta.spends * segment.length()};
    }
...
private:
    ...
    public:
        ...
        Data ProcessPartial(const NodeHolder&, IndexSegment, const Data& left_result, const Data& right_result) const {
            return {left_result.incomes + right_result.incomes, left_result.spends + right_result.spends};
        }
    };
    ...
    public:
        ...
        void ProcessPartial(const NodeHolder& node, IndexSegment) const {
            node->data = {(node->left ? node->left->data.incomes : Data().incomes) +
                    (node->right ? node->right->data.incomes : Data().incomes),
                    (node->left ? node->left->data.spends : Data().spends) +
                    (node->right ? node->right->data.spends : Data().spends)
                    };
        }
...
struct ComputeIncomeRequest : ReadRequest<double> {
    ...
    double Process(const BudgetManager& manager) const override {
        return manager.ComputeSum(MakeDateSegment(date_from, date_to)).ComputeSum();
    }
...
struct SpendRequest : ModifyRequest {
    ...
    void ParseFrom(string_view input) override {
        date_from = Date::FromString(ReadToken(input));
        date_to = Date::FromString(ReadToken(input));
        spends = ConvertToInt(input);
    }

    void Process(BudgetManager& manager) const override {
        const auto date_segment = MakeDateSegment(date_from, date_to);
        const double daily_income = spends * 1.0 / date_segment.length();
        manager.AddBulkOperation(date_segment, BulkMoneyAdder{0.0, daily_income});
    }
