...
public:
    ...
    Person(const std::string& fname_, const std::string& lname_, const int& year){
        fname[year] = fname_;
        lname[year] = lname_;
    }
    Person(const char * const fname_, const char * const lname_, const int& year){
        fname[year] = fname_;
        lname[year] = lname_;
    }
    ...
    {
        if(year >= fname.begin()->first)
            fname[year] = first_name;
    }

    void ChangeLastName(int year, const std::string &last_name)
    {
        if(year >= lname.begin()->first)
